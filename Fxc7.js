	...
	frhan.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				success: '*SUCESSO...*',
				error: {
					stick: ' *Bem, falhe, tente novamente*',
					Iv: '*ᴍᴀᴀꜰ ʟɪɴᴋ ᴛɪᴅᴀᴋ ᴠᴀʟɪᴅ!!*'
				},
				only: {
					group: '*[❗] Este comando só pode ser usado em grupos!*',
					benned: '*Desculpe, seu número está banido. Entre em contato com o proprietário para retirar seu ban*',
					ownerG: '*[❗] Proprietário? Este é um recurso especial para o proprietário do Grupo!*',
					ownerB: '*[❗] Proprietário? Este é um recurso especial para o proprietário do bot!* ',
					premium: '*[❗] ESTE PEDIDO É SO PARA *USUÁRIOS PREMIUMS!!*',
					userB: `Salve ${pushname2} Você não está registrado no banco de dados. Digite \n${prefix}daftar`,
					admin: '*[❗] Este comando só pode ser usado por administradores de grupo!*',
					Badmin: '*[❗] Este comando só pode ser usado quando o bot se torna administrado!*',
					publikG: `*Desculpe, o bot foi privatizado pelo proprietário*\n*Para mais detalhes, digite*\n*${prefix}infobot*`
				}
			}

			const botNumber = frhan.user.jid
			const ownerNumber = ["553398091185@s.whatsapp.net"] // número do proprietário
		...
		case 'daftar':
			...
			if (!isPublic) return reply(mess.only.publikG)
				...
				case 'info':
					...
					user.push(sender)
					uptime = process.uptime()
					teks = `➽ *Nome Do Bot* : ${me.name}\n➽ *Dono Do Bot* : @${ownerNumber}\n➽ *prefix* : | ${prefix} |\n➽ *Total De Contatos Bloqueado* : ${blocked.length}\n➽ *Ativo Desde* : ${kyun(uptime)}\n\n➽ *Total de usuários* : ${user.length} User\n➽ *Instagram* : https://instagram.com/pwtx.ff?igshid=13dwo5oa0q519\n➽ *Special Thanks To* :\n\n➽ Allah SWT \n➽ MhankBarBar\n➽ Nurutomo\n➽ Monurios`
...
//creator
				...
				if (!isPublic) return reply(mess.only.publikG)
				if (isLimit(sender)) return reply(limitend(pushname2))
			if (args.length < 1) return reply(`${name} O que escrever, mano??`)
			reply(mess.wait)
					tulis = body.slice(7)
				  nama = tulis.split("/")[0];
					kelas = tulis.split("/")[1];
...
//akhir kreator
		    ...
		    case 'map':
			    ...
			    if (!isPublic) return reply(mess.only.publikG)
					...
					mentions(`╔═══✪ Tag By *${pushname2}* ✪══`+ teks +'╚═══〘 TIAGO BOT 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
			    if (!isPublic) return reply(mess.only.publikG)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 TIAGO BOT 〙', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				if (!isPublic) return reply(mess.only.publikG)
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('O que você deseja adicionar??')
					...
					try {
						...
						console.log('Error :', e)
						reply('Falha ao adicionar destino, talvez porque era privado')
					}
					break
			    case 'kick':
			    if (isBanned) return reply(mess.only.benned)    
				...
				if (!isPublic) return reply(mess.only.publikG)
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedidos aceitos, você chega lá :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
				...
				if (!isOwner) return reply(mess.only.ownerB)
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('A tag alvo que você deseja')
